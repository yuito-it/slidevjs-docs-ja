...
const formattedTag = computed(() => {
  const tag = props.tag.replace(/\b(API|CLI)\b/gi, (m) => m.toUpperCase()).replace(/-/g, " ");
...
<template>
  <a
    ...
    class="feature-tag flex gap-1 items-center"
  >
    {{ formattedTag }}
    <button
      class="flex items-center op-70 hover:bg-gray-200/20 hover:op90 rounded-full mr--1"
      @click="emit('remove')"
    >
  ...
  <a
    v-else
    class="feature-tag"
    :href="withBase(`/features/#tags=${tag}`)"
  >
    {{ formattedTag }}
  </a>
</template>
